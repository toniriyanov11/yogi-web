...
router.insertPembelianTypeOthers = function(data) {
    return new Promise((resolve, reject) => {
        database.getConnection().query(`CALL P_PEMBELIAN_LAINNYA(?,?,?,?,?,?,?,?,?)`,[data.tanggal,data.nama,data.jumlah,data.total,data.statusBayar,data.ket,data.hutang,data.tglSekarang, data.jenisPembelian],(err,results) => {
            if (err) {
                database.getConnection().query(`ROLLBACK;`)
                console.log(err)
                return reject(err)
