...
router.insertBarangJadi = function(data) {
    console.log(data)
    return new Promise((resolve, reject) => {
        database.getConnection().query(`CALL P_BARANG_JADI(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`,[data.tanggal, data.nama, data.hargaPerPcs, data.ket , data.tglSekarang , data.item, data.biayaTambahan.tieDye, data.biayaTambahan.label, data.biayaTambahan.handtag, data.biayaTambahan.packing, data.biayaTambahan.dllProduksi, data.biayaTambahan.dllAksesoris, data.jumlah, data.hargaPokok, data.grandTotalBiayaTambahanProduksi],(err,results) => {
            if (err) {
                database.getConnection().query(`ROLLBACK;`)
                console.log(err)
                return reject(err)
